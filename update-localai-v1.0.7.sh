...
printf '\n==============================================\n'
printf ' KITZ Local AI v1.0.7 - macOS mktemp Fix\n'
printf '==============================================\n\n'
...
if ! curl -fsS --max-time 5 http://127.0.0.1:8787/health >/dev/null 2>&1; then
  echo '[v1.0.7] Agent Core is offline; starting KITZ services...'
  kitz-start || true
fi
...
echo '[v1.0.7] Agent Core: ONLINE'

echo '[v1.0.7] Installing streaming compatibility bridge...'
tmp_bridge="$(mktemp "${TMPDIR:-/tmp}/kitz-stream-bridge.XXXXXX")"
tmp_installer="$(mktemp "${TMPDIR:-/tmp}/kitz-localai-installer.XXXXXX")"
capfile="$(mktemp "${TMPDIR:-/tmp}/kitz-localai-capabilities.XXXXXX")"
stream_body="$(mktemp "${TMPDIR:-/tmp}/kitz-localai-stream.XXXXXX")"
trap 'rm -f "$tmp_bridge" "$tmp_installer" "$capfile" "$stream_body"' EXIT

curl -fsSL https://raw.githubusercontent.com/kitz-labs/kitz-local-ai-bootstrap/v1.0.7/release-overrides/localai_stream_bridge.py -o "$tmp_bridge"
...
echo '[v1.0.7] Waiting for streaming bridge on :8788...'
i=0
while [ "$i" -lt 30 ]; do
...
echo '[v1.0.7] Streaming bridge: ONLINE'

echo '[v1.0.7] Pointing LocalAI kitz-agent to streaming bridge...'
curl -fsSL https://raw.githubusercontent.com/kitz-labs/kitz-local-ai-bootstrap/v1.0.7/release-overrides/install_localai_bridge.py -o "$tmp_installer"
uv run --no-project --python 3.12 python "$tmp_installer" --no-restart --no-chat-test

echo '[v1.0.7] Restarting LocalAI safely...'
osascript -e 'tell application "LocalAI" to quit' >/dev/null 2>&1 || true
sleep 2
pkill -x local-ai >/dev/null 2>&1 || true
open -a LocalAI

echo '[v1.0.7] Waiting for LocalAI on :8080...'
i=0
while [ "$i" -lt 90 ]; do
...
print('[v1.0.7] kitz-agent capability: FLAG_CHAT')
PY

echo '[v1.0.7] Running real LocalAI streaming chat test...'
...
echo '[v1.0.7] VERIFIED: LocalAI -> kitz-agent -> streaming bridge -> Agent Core returned HTTP 200 + SSE'
printf '\n==============================================\n'
